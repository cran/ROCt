...
allcause.ROCt<-function(times, failures, variable, pro.time, cut.off, knn=FALSE, lambda=NULL) {
...
.survie.lambda<-function(x) {
 sum(.survie.temp * (.temp.data$variable>x)) / .n }

.survie.marginale<-sum(.survie.temp) / .n

.temp.se<-function(x) {
 (1-sum(.temp.data$variable<=x)/.n-.survie.lambda(x))/(1-.survie.marginale) }
 
.temp.sp<-function(x)
{ 1-(.survie.lambda(x)/.survie.marginale) }
...
net.ROCt<-function(times, failures, variable, p.age, p.sex, p.year, rate.table, pro.time, cut.off, knn=FALSE, lambda=NULL) {
...
.pas <- round(lambda * dim(.temp.data)[1])
